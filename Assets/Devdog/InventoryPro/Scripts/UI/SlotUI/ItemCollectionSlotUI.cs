...
namespace Devdog.InventoryPro
{
    ...
    public partial class ItemCollectionSlotUI : ItemCollectionSlotUIBase, IBeginDragHandler, IEndDragHandler, IDragHandler, IPointerUpHandler, IPointerDownHandler, IPointerEnterHandler, IPointerExitHandler
    {
        ...
        public virtual void OnPointerDown(PointerEventData eventData)
        {
            ...
            if (InventorySettingsManager.instance.settings.useContextMenu && InventorySettingsManager.instance.settings.triggerContextMenuKeys.AllPressed(eventData, InventoryActionInput.EventType.OnPointerDown))
            {
                if (item != null)
                {
                    TriggerContextMenu();
...
//            lastDownTime = 0f;
            ...
            if (InventorySettingsManager.instance.settings.useContextMenu && InventorySettingsManager.instance.settings.triggerContextMenuKeys.AllPressed(eventData, InventoryActionInput.EventType.OnPointerUp))
            {
                if (item != null)
                {
                    TriggerContextMenu();
        ...
        {
            if (InventorySettingsManager.instance.settings.unstackKeys.AllPressed(tap, eventUsed, eventData))
            {
                TriggerUnstack(itemCollection);
                return true;
            }

            if (InventorySettingsManager.instance.settings.useItemKeys.AllPressed(tap, eventUsed, eventData))
            {
                TriggerUse();
                return true;
            }

            if (InventorySettingsManager.instance.settings.triggerContextMenuKeys.AllPressed(tap, eventUsed, eventData))
            {
